    ...
    if(date1<date2){
        ...
        for (auto it = recordMap.begin(); it != recordMap.end(); ++it) {
            ...
            if(date1<datetmp&&datetmp<date2){
                ...
                datetmp.show();
                if(inputMod) std::cout<<'\t'<<ar.getId()<<'\t'<<ar.getAmount()<<'\t'<<ar.getBalance()<<endl;
...
accountException::accountException(const std::string& message,Account *account): std::runtime_error(message),account(account) {}

SavingsAccount::SavingsAccount(Date date,std::string id,double rate) : rate(rate){
    changeLastDate(date);
    logId(id);
    if(inputMod) date.show();
    if(inputMod) std::cout<<'\t'<<"#"<<id<<" created"<<endl;
}
...
void SavingsAccount::deposit(Date date,double amount,string note) {
    ...
    addBalance(amount);
    if(inputMod) date.show();
    if(inputMod) std::cout<<'\t'<<"#"<<getId()<<'\t'<<amount<<'\t'<<getBalance()<<'\t'<<note<<endl;
...
void SavingsAccount::settle(Date date){
    ...
    if(inputMod) date.show();  
    if(inputMod) std::cout<<'\t'<<"#"<<getId()<<'\t'<<floor(((acc.getAccumulation()/366)*rate) * 100 + 0.5) / 100<<'\t'<<getBalance()<<'\t'<<"interest"<<endl;
...
void SavingsAccount::withdraw(Date date,double amount,string note){
    ...
    if(amount>getBalance()){
        throw accountException("余额不足",this);
    }else{
    ...
    //lastDate = date;
    addBalance(-amount);
    if(inputMod) acc.getLastDate().show();
    if(inputMod) std::cout<<'\t'<<"#"<<getId()<<'\t'<<"-"<<amount<<'\t'<<getBalance()<<'\t'<<note<<endl;
    addTotalBalance(-amount);
...
void CreditAccount::withdraw(Date date,double amount,string note){
    ...
    if(amount>getAvailableCredit()){
        throw accountException("额度不足",this);
    }else{
...
void CreditAccount::settle(Date date){
    ...
    if((floor(((acc.getAccumulation())*rate) * 100 + 0.5) / 100)!=0){
        if(inputMod) date.show();
        if(inputMod) std::cout<<'\t'<<"#"<<getId()<<'\t'<<(floor(((acc.getAccumulation())*rate) * 100 + 0.5) / 100)<<'\t'<<getBalance()<<'\t'<<"interest"<<endl;
    }
